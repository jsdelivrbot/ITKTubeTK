...
::m_Extreme( double * extX, double * extVal )
{
  ...
  for( iter = 0; iter < m_MaxIterations; iter++ )
    {
    double xm = 0.5 * ( a+b );
    double tol1 = m_Tolerance * vnl_math::abs( x ) + m_Epsilon;
    double tol2 = 2.0 * tol1;
    if( vnl_math::abs( x-xm ) <= ( tol2 - 0.5*( b-a ) ) )
      {
      *extX = x;
      *extVal = maxSign*fx;
      return true;
      }
    if( vnl_math::abs( e ) > tol1 )
      {
      d1 = 2.0*( b-a );
      d2 = d1;
      if( dw != dx )
      ...
      if( ok1 || ok2 )
        ...
        if( ok1 && ok2 )
          {
          d = ( vnl_math::abs( d1 ) < vnl_math::abs( d2 ) ? d1 : d2 );
          }
        else
          {
          if( ok1 )
        ...
        if( vnl_math::abs( d ) <= vnl_math::abs( 0.5 * olde ) )
          {
          u = x+d;
