...
int itktubeRadiusExtractor2Test2( int argc, char * argv[] )
{
  ...
  for( unsigned int mcRun=0; mcRun<numMCRuns; mcRun++ )
    ...
    for( unsigned int i=0; i<numPoints; i++ )
      ...
      if( vnl_math::abs( pntIter->GetTangent().GetVnlVector().magnitude()-1 )
        > 0.01 )
        {
      ...
      if( vnl_math::abs( pntIter->GetNormal1().GetVnlVector().magnitude()-1 )
        > 0.01 )
        {
      ...
      if( vnl_math::abs( pntIter->GetNormal2().GetVnlVector().magnitude()-1 )
        > 0.01 )
        {
      ...
      if( vnl_math::abs( dot_product( pntIter->GetTangent().GetVnlVector(),
      ...
      if( vnl_math::abs( dot_product( pntIter->GetNormal1().GetVnlVector(),
        pntIter->GetNormal2().GetVnlVector() ) ) > 0.001 )
        {
        std::cout << "Point: " << i
    ...
    for( unsigned int i=0; i<numPoints; i++ )
      {
      double diff = vnl_math::abs( pntIter->GetRadius() - idealR[i] );
      avgDiff += diff;
      if( diff > maxDiff )
        {
        maxDiff = diff;
