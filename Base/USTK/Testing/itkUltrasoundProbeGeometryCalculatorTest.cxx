...
int itkUltrasoundProbeGeometryCalculatorTest( int argc, char * argv[] )
{
  ...
  if( vnl_math::abs( ultrasoundProbeOrigin[0] - 331.5 ) > tolerance ||
      vnl_math::abs( ultrasoundProbeOrigin[1] - -528.4 ) >  tolerance )
    {
    std::cerr << "Did not find the correct probe origin!" << std::endl;
    return EXIT_FAILURE;
    }

  if( vnl_math::abs( startOfAcquisitionRadius - 555.6 ) > tolerance )
    {
