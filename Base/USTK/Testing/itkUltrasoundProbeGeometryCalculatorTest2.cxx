...
int itkUltrasoundProbeGeometryCalculatorTest2( int argc, char * argv[] )
{
  ...
  if( vnl_math::abs( ultrasoundProbeOrigin[0] - 354.9 ) > tolerance ||
      vnl_math::abs( ultrasoundProbeOrigin[1] - -214.8 ) >  tolerance ||
      vnl_math::abs( ultrasoundProbeOrigin[2] - 185.7 ) >  tolerance )
    {
    std::cerr << "Did not find the correct probe origin!" << std::endl;
    return EXIT_FAILURE;
    }

  if( vnl_math::abs( startOfAcquisitionRadius - 74.5 ) > tolerance )
    {
